...
```{r}
mrr_income_gradients <- setDT(read_excel(here('data_in', 'mrr_income_gradients.xlsx')))[, mrr := irr]
...
create_plot <- function(sukup_in){
  ...
  .[, psy := factor(psy, labels = c('Yes', 'No'))] %>%
  ...
  plot_ly( x = ~x, y = ~mrr, type = 'scatter', error_y = list(array=~se), group=~psy, color=~psy, mode = 'lines+markers') %>% 
    layout(
      yaxis = list(
        range=
          if (input$psy_type %in% c('inpatient', 'outpatient')){
            c(0.5, 2.1)
          } else {
            c(0.1, 2.4)
          }
...
radioButtons(
  'model_n', h4('Model'),
  choices = list("1: Age-group, Calender Year" = 1, 
                 "2: 1 + Urbanicity" = 2,
                 "3: 2 + Education" = 3, 
                 "4: 3 + Living Alone" = 4,
                 "5: 4 + Ecnomic Activity" = 5, 
                 "6: 4 For Employed Only" = 6,
                 "7: 4 For Non-Employed" = 7, 
                 "8: 1+ Ecnomic Activity" = 8,
                 "9: 1 For Employed Only" = 9
                 ),
  selected = 1)
